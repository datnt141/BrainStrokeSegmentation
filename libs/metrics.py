...
class SegMetrics_2D:
    ...
    def get_indicator(i_label=None,i_pred=None,i_object_index=1):
        ...
        cmps = ListTuples.compare(i_label.shape,i_pred.shape)
        assert sum(cmps)==0
    ...
    def get_metrics(self,i_label=None,i_pred=None):
        assert isinstance(i_label, np.ndarray)
        assert isinstance(i_pred, np.ndarray)
        if len(i_label.shape)==2:
            i_label = np.expand_dims(i_label,axis=-1)
        else:
            assert i_label.shape[-1]==1
        if len(i_pred.shape)==2:
            i_pred = np.expand_dims(i_pred,axis=-1)
        else:
            assert i_pred.shape[-1]==1
        assert len(i_label.shape) == len(i_pred.shape),'Got values: {} vs {}'.format(i_label.shape,i_pred.shape)
        cmps = ListTuples.compare(i_label.shape, i_pred.shape)
        assert sum(cmps) == 0
        self.get_indicators(i_label=i_label,i_pred=i_pred)
        measures = list()
    ...
    def eval(self,i_labels=None,i_preds=None,i_object_care=False):
...
class SegMetrics_3D:
    ...
    def get_measures(self, i_labels=None,i_preds=None,i_object_index=1):
        ...
        for index,label in enumerate(i_labels):
            ...
            TN,FP,TP,FN = SegMetrics_2D.get_indicator(i_label=label,i_pred=pred,i_object_index=i_object_index)
            TNs += TN
            FPs += FP
            TPs += TP
            FNs += FN
...
if __name__ == '__main__':
    ...
    exampler = SegMetrics_2D(i_num_classes=2,i_care_background=False)
