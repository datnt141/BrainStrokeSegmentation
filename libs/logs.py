...
class Logs:
    ...
    @classmethod
    def del_log(cls):
        log_path = cls.init_log_path()
        cls.sure_to_delete = True
        cls.delete(i_file_path=log_path)
        return True
    @classmethod
    def delete(cls, i_file_path=None):
        """Delete a file in filesystem"""
        if os.path.exists(i_file_path):
            if cls.sure_to_delete:
                os.remove(i_file_path)
                return True
            else:
                print('Unable to delete {} file'.format(i_file_path))
                print('Set FileUtils.sure_to_delete flag to True to enable deletion')
                return False
        else:
            print("Warning: Cannot delete anything because '{}' does not exist".format(i_file_path))
            return False
    @classmethod
    ...
    def log_cls_params(cls,i_cls):
        """i_cls is self or an instance of an class"""
